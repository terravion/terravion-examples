...
class ProductLib(object):
    ...
    def make_thermal(self, output_filepath):
        ...
        t = np.multiply(t, image_tools.UINT8)
        t = np.asarray(t, dtype=np.uint8)
        t = np.clip(t, 0, image_tools.UINT8).astype(np.uint8)
    ...
    def make_ndvi(self, output_filepath):
        ...
        ndvi = self.calculate_ndvi(RED, NIR,
                                   ...
                                   beta=sndvi_beta,
                                   low=0, high=1.0)
        ndvi *= image_tools.UINT8

        self.log.debug('Clipping data to 8bit as a precaution')
        ndvi = np.clip(ndvi, 0, image_tools.UINT8).astype(np.uint8)
    ...
    def calculate_ndvi(self, red, nir, alpha=0.0, beta=1.0, low=0.0, high=1.0):
        ...
        ir_ratio[np.isnan(ir_ratio)] = 0
        ir_ratio[np.isinf(ir_ratio)] = 0

        self.log.debug('VIS/NIR ratio mean: %s', str(np.mean(ir_ratio)))
        self.log.debug('VIS/NIR ratio std: %s', str(np.std(ir_ratio)))

        # Compute ndvi
        ndvi = ((1 - (ir_ratio)) / (1 + (ir_ratio))) + alpha
